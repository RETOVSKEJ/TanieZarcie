...
async function scrapeProducts() {
    ...
    const countOfCategories = await page.$$eval("li.swiper-slide.k5LBY", (elems) => {
    ...
    for (let i = 3; i <= countOfCategories; i++, counter.categoryId++) {
        ...
        const categoryName = await page.$eval(categoryNameSelector, (el) => el.textContent)
        const countofProducts = await page.$$eval(productsSelector, (products) => {
        ...
        for (let j = 1; j <= countofProducts; j++, counter.productId++) {
            ...
            const promise = await page.evaluate(
                (counter, nameSelector, priceSelector) => {
                    ...
                    return {id: counter.productId, categoryId: counter.categoryId, name, price}
                },
                counter,
                nameSelector,
                priceSelector
        ...
        CATEGORIES = [...CATEGORIES, {id: counter.categoryId, name: categoryName}]
